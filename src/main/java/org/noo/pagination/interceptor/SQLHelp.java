...
public class SQLHelp {
	...
	public static int getCount(final String sql, final Connection connection,
		...
		String noOrderBySql = removeOrderByPart(sql);
		String countSql = "select count(1) from (" + noOrderBySql + ") as tmp_count";
		//oracle单独处理
		if (dialect instanceof OracleDialect) {
			countSql = "select count(1) as tmp_count from (" + noOrderBySql + ")";
		}
	 ...
	 * @return
	 */
	static String removeOrderByPart(String sql) {
		String loweredString = sql.toLowerCase();
		int orderByIndex = loweredString.indexOf("order by");
		if (orderByIndex != -1) {
			// if we find a new "order by" then we need to ignore
			// the previous one since it was probably used for a subquery
			return StringUtils.remove(loweredString, loweredString.substring(orderByIndex));
		} else {
	...
	public static String generatePageSql(String sql, int start, int pageSize, Dialect dialect) {
		if (dialect.supportsLimit()) {
			return dialect.getLimitString(sql, start, pageSize);
		} else {
			return sql;
